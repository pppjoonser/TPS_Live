...
EBTNodeResult::Type UBTTask_TurnToTarget::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	LookVector.Z = 0.0f;
	TargetRotaion = FRotationMatrix::MakeFromX(LookVector).Rotator();
...
void UBTTask_TurnToTarget::TickTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
	...
	ControllingPawn->SetActorRotation(FMath::RInterpTo(CurrentRotation, TargetRotaion, DeltaSeconds, TurnSpeed));
	if (ControllingPawn->GetActorRotation() == TargetRotaion)
	{
